...
export default function PreordersPage() {
  ...
  const updateUrl = useCallback(
    (newFilter?: string, newSort?: string, newOrder?: string, newPage?: number) => {
      const params = new URLSearchParams();
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                <div className="p-3">
                  ...
                  <div className="border-t border-gray-200 mt-3 pt-3">
                    ...
                    >
                      {order === 'desc' ? '↓' : '↑'}
                      <span>{order === 'desc' ? 'Descending' : 'Ascending'}</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            <>
              <table className="w-full">
                <thead className="border-b border-gray-200 bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left">
                      <input
                        type="checkbox"
                        checked={selectedRows.size === preorders.length && preorders.length > 0}
                    ...
                    <th className="px-6 py-3 text-left font-semibold text-gray-900">Starts at</th>
                    <th className="px-6 py-3 text-left font-semibold text-gray-900">Ends at</th>
                ...
                <tbody>
                  {preorders.map((preorder) => (
                    <tr key={preorder.id} className="border-t border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 font-medium text-gray-900">{preorder.name}</td>
                      <td className="px-6 py-4 text-gray-700">{preorder.products}</td>
                      <td className="px-6 py-4 text-gray-700">{preorder.preorderWhen}</td>
                      <td className="px-6 py-4 text-gray-700 text-sm">{formatDate(preorder.startsAt)}</td>
                      <td className="px-6 py-4 text-gray-700 text-sm">
                        {preorder.endsAt ? formatDate(preorder.endsAt) : '-'}
                      </td>
                      <td className="px-6 py-4">
                        <button
                          onClick={() => toggleStatus(preorder.id, preorder.status)}
                        ...
                        >
                          <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                              preorder.status ? 'translate-x-6' : 'translate-x-1'
                            }`}
                          />
                        </button>
                      </td>
              ...
              <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
                <p className="text-sm text-gray-600">
                  Showing {(page - 1) * 8 + 1} to {Math.min(page * 8, total)} from {total}
                </p>
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      ...
                      updateUrl(filter, sortBy, order, Math.min(totalPages, page + 1));
                    }}
